...
use Modules\Post\Http\Requests\StorePost;
use Modules\Post\Http\Requests\UpdatePost;
...
class PostController extends Controller
{
     ...
     * @return Renderable
     */
    public function store(StorePost $request, ImageService $imageService)
    {
        $inputs = $request->all();
        ...
        //TODO author_id AFTER CREATE LOGIN METHOD FIXED !
        $inputs['author_id'] = 1;

        $post = Post::create($inputs);
        return redirect()->route('admin.content.post.index')->with('swal-success', ' پست شما با موفقیت ایجاد شد');
    }
     ...
     * @return Renderable
     */
    public function update(UpdatePost $request, Post $post, ImageService $imageService)
    {
        ...
        $post->update($inputs);
        return redirect()->route('admin.content.post.index')->with('swal-success', ' پست شما با موفقیت اپدیت شد');
    }
    /**
