...
class LoginRegisterController extends Controller
{
    ...
    public function loginRegister(LoginRegisterRequest $request)
    {
        ...
        elseif (preg_match('/^(\+98|98|0)9\d{9}$/', $inputs['id'])) {
            $type = 0; //code for mobile

            //change format for register or login mobile
            ...
            $errorText = 'شناسه ورودی شما شماره موبایل و یا ایمیل نیست';
        ...
        return redirect()->route('auth.customer.login-confirm', $token);
    }
    ...
    public function loginConfirmForm($token)
    {
        ...
        if (empty($otp)) {
            return redirect()->route('auth.customer.login-register-form')->withErrors(['id' => 'فیلد وارد شده صحیح نمیباشد']);
        }
    ...
    public function loginConfirm($token, LoginRegisterRequest $request)
    {
        ...
        if (empty($otp)) {
            return redirect()->route('auth.customer.login-register-form', $token)->withErrors(['id' => 'ادرس وارد شده نامعتبر میباشد']);
        }
        //if otp not match
        if ($otp->otp_code !== $inputs['otp']) {
            return redirect()->route('auth.customer.login-confirm-form', $token)->withErrors(['otp' => 'کد وارد شده صحیح میباشد']);
        }
