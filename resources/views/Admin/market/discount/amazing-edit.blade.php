    ...
    <section class="row">
        <section class="col-12">
            <section class="main-body-container ">
                ...
                <section>
                    <form action="{{ route('admin.market.discount.amazingSaleUpdate', $amazing->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <section class="row d-flex justify-content-between">
                                    ...
                                    <select name="product_id" id="product_id" class="form-control form-control-sm">
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}"
                                                @if (old('product_id', $amazing->product_id) == $product->id) selected @endif>{{ $product->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <section class="col-12 col-md-6">
                                <div class="form-group ">
                                    ...
                                    <input type="text" value="{{ old('percentage', $amazing->percentage) }}"
                                        name="percentage" class="form-control form-control-sm">
                                </div>
                                @error('percentage')
                                    <span class="alert_required bg-danger rounded text-white p-1" role="alert">
                                        <strong>
