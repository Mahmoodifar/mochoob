                        ...
                        <tbody>
                            @foreach ($payments as $payment)
                                <tr>
                                    ...
                                    <td>
                                        @if ($payment->type == 0) انلاین  @elseif ($payment->type == 1) افلاین @else در محل @endif
                                    </td>
                                    <td class="width-22-rem text-left">
                                        <a href="{{ route('admin.market.payment.show', $payment->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> مشاهده</a>

                                        @if ($payment->status == 2)
                                            <a href="{{ route('admin.market.payment.confirm', $payment->id) }}"
                                                class="btn btn-success btn-sm"><i class="fa fa-close"></i> تایید شده
                                            </a>
                                        @else
                                            <a href="{{ route('admin.market.payment.canceled', $payment->id) }}"
                                                class="btn btn-warning btn-sm"><i class="fa fa-close"></i> باطل
                                                کردن</a>
                                        @endif
